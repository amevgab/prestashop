...
<div id="wishlist_block" class="block account">
	<h4 class="title_block">
		{assign var="blockwishlist" value=$link->getModuleLink('blockwishlist', 'mywishlist', array(), true)}
		<a href="{$blockwishlist|escape:'html':'UTF-8'}" title="{l s='My wishlists' mod='blockwishlist'}" rel="nofollow">{l s='Wishlist' mod='blockwishlist'}</a>
	</h4>
	<div class="block_content">
		...
		{if $wishlist_products}
			...
			{foreach from=$wishlist_products item=product name=i}
				<dt class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
					...
					{assign var="cart_block_product_name" value=$link->getProductLink($product.id_product, $product.link_rewrite, $product.category_rewrite)}
					<a class="cart_block_product_name"
					href="{$cart_block_product_name|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">{$product.name|truncate:30:'...'|escape:'html':'UTF-8'}</a>
				...
				<dd class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
					{assign var="productDetail" value=$link->getProductLink($product.id_product, $product.link_rewrite, $product.category_rewrite)}
					<a href="{$productDetail|escape:'html':'UTF-8'}" title="{l s='Product detail'}">{$product.attributes_small|escape:'html':'UTF-8'}</a>
				</dd>
				{/if}
			{/foreach}
			</dl>
		...
		{/if}
			{assign var="mywishlist" value=$link->getProductLink('blockwishlist', 'mywishlist', array(), true)}
			<a href="{$mywishlist|escape:'html':'UTF-8'}" title="{l s='My wishlists' mod='blockwishlist'}" rel="nofollow">&raquo; {l s='My wishlists' mod='blockwishlist'}</a>
		</p>
	</div>
</div>
